...
describe('Linked list tests', () => {
  ...
  it('should check if the default array is rendered', () => {
    cy.get('div[class*="circle_circle"]').as('circle').should('have.length', 4);
    ...
    cy.get('div[class*="circle_content"] div[class*="text"]:last-of-type').eq(3).should('have.text', 'tail');
  });
  ...
  it('should correctly add an element at an index', () => {
    ...
    cy.get('div[class*="circle_circle"]').as('circle');
    cy.get('div[class*="circle_content"]').as('circleContent');
  ...
  it('should delete an element from the head', () => {
    ...
    cy.get('div[class*="changing"]').eq(0).should('have.css', 'border-color', changingColor).invoke('text').should('not.be.empty');
    cy.tick(SHORT_DELAY_IN_MS);
  ...
  it('should delete an element from the tail', () => {
    ...
    cy.get('div[class*="changing"]').last().should('have.css', 'border-color', changingColor).invoke('text').should('not.be.empty');
    cy.tick(SHORT_DELAY_IN_MS);
    cy.get('@circle').should('have.length', 3);
  });
